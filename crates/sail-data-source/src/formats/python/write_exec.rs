        ...
        let stream = futures::stream::once(async move {
            ...
            for p in 0..num_partitions {
                ...
                let write_future = async move {
                    ...
                    // Execute write for this partition
                    executor_clone
                        .execute_write(&pickled_writer_clone, is_arrow, schema_clone, batches)
                        .await
                };
            ...
            for result in results {
                match result {
                    Ok(write_result) => {
                        if let Some(msg) = write_result.commit_message {
                            commit_messages.push(msg);
                        } else {
                            // Push empty message for None case
                            commit_messages.push(vec![]);
                        }
                    }
                    Err(e) => {
                        had_failure = true;
                        ...
                        // Push empty message for failed partition
                        commit_messages.push(vec![]);
                    }
                }
            }
