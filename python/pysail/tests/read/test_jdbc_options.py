...
import pytest
from pysail.read.jdbc_options import NormalizedJDBCOptions, normalize_session_init
from pysail.read.exceptions import InvalidOptionsError
...
class TestNormalizedJDBCOptions:
    ...
    def test_both_dbtable_and_query_raises_error(self):
        ...
        with pytest.raises(
            InvalidOptionsError,
            match="Cannot specify both 'dbtable' and 'query'"
        ):
            NormalizedJDBCOptions.from_spark_options(options)
    ...
    def test_partition_options(self):
        ...
        assert opts.lower_bound == 1
        assert opts.upper_bound == 1000
        assert opts.num_partitions == 10
    ...
    def test_invalid_bounds_fails_validation(self):
        ...
        with pytest.raises(
            InvalidOptionsError,
            match="lowerBound .* must be less than upperBound"
        ):
            opts.validate()
    ...
    def test_invalid_engine_raises_error(self):
        ...
        with pytest.raises(
            InvalidOptionsError,
            match="Invalid engine: invalid_engine"
        ):
            NormalizedJDBCOptions.from_spark_options(options)
...
class TestSessionInitNormalization:
    ...
    def test_reject_insert_statement(self):
        ...
        with pytest.raises(
            InvalidOptionsError,
            match="sessionInitStatement cannot start with INSERT"
        ):
    ...
    def test_reject_update_statement(self):
        ...
        with pytest.raises(
            InvalidOptionsError,
            match="sessionInitStatement cannot start with UPDATE"
        ):
            normalize_session_init(stmt)
    ...
    def test_reject_delete_statement(self):
        ...
        with pytest.raises(
            InvalidOptionsError,
            match="sessionInitStatement cannot start with DELETE"
        ):
            normalize_session_init(stmt)
    ...
    def test_reject_drop_statement(self):
        ...
        with pytest.raises(
            InvalidOptionsError,
            match="sessionInitStatement cannot start with DROP"
        ):
            normalize_session_init(stmt)
